    ...
    <div className="text-center mt-8 w-full ">
      {books.length === 0 ? (
        <p className="text-sm mt-4">
          Add books to {shelfName} - <a href="/browse" className="text-blue-500">Browse here</a>
        </p>
      ) : (
        <div className="flex flex-col gap-2 mt-4">
          <div className={`grid ${shelfName === 'To Read' ? 'grid-cols-6' : shelfName === 'Currently Reading' ?  'grid-cols-8' : 'grid-cols-7'} gap-4 px-16 py-4 font-bold bg-orange-300 text-brown-700`}>
            <div></div>
            <p>TITLE & AUTHOR</p>
          ...
          {books.map((book) => (
            <div key={book.id} className={`grid ${shelfName === 'To Read' ? 'grid-cols-6' : shelfName === 'Currently Reading' ?  'grid-cols-8' : 'grid-cols-7'} gap-4 place-items-center px-16 py-8 bg-orange-600 text-brown-100`}>
              ...
              {shelfName !== "To Read" && (
                <div className="flex space-x-1">
                  {Array.from({ length: 5 }, (_, index) => (
                    <img
                      ...
                      onMouseEnter={() => setHoveredRatings((prev: any) => ({ ...prev, [book.id]: index + 1 }))}  
                      onMouseLeave={() => setHoveredRatings((prev: any) => ({ ...prev, [book.id]: null }))}     
                ...
                <div className="flex items-center space-x-2 mt-4">
                    ...
                    onChange={(e) => handleUpdateBook(book.id, 'pagesRead', e.target.value)} 
                    />
                    <p className="text-lg">{book.pagesTotal ? `/${book.pagesTotal}` : '/0'}</p>
                </div>
                )}
