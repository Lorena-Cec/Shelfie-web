...
const NavNewUser: React.FC<NavigationProps> = ({currentPage}) => {
  ...
  return (
    <nav className="flex justify-between items-center py-6 px-10 bg-orange-400 text-white">
        <h1 className='text-4xl text-brown-100 font-extrabold tracking-tighter'>Shelfie</h1>
        <div className='flex items-center gap-4'>
            {pages.map((page, index) =>(
                <div key={page.pageNumber} className='flex gap-4 items-center'>
                    <Link href = {page.link}>
                        <p className={`${currentPage === index + 1 ? 'text-orange-100 font-extrabold' : 'font-normal'}`}>
        ...
        <p className='text-4xl text-orange-400 font-extrabold tracking-tighter'>Shelfie</p>
