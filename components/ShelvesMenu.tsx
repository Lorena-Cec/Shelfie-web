...
const moveBookToShelf = async (bookId: string, newShelf: string, updatedBooks: any[]) => {
  try {
    if (!userId) return; 
    const currentShelfRef = doc(db, 'users', userId, 'shelves', selectedShelf);
    const newShelfRef = doc(db, 'users', userId, 'shelves', newShelf);

    
    const currentBooks = updatedBooks.filter((book: any) => book.id !== bookId);
    ...
    const newShelfSnap = await getDoc(newShelfRef);
    const newShelfBooks = newShelfSnap.exists() ? newShelfSnap.data().books || [] : [];

    
    const bookToMove = updatedBooks.find((book: any) => book.id === bookId);
    await setDoc(newShelfRef, { books: arrayUnion(bookToMove) }, { merge: true });
  } catch (error) {
    ...
    const handleUpdateBook = async (bookId: string, field: string, value: any) => {
      try {
        ...
        if (shelfSnap.exists()) {
          ...
          } else if (field === 'readDate' && value && (selectedShelf === 'To Read' || selectedShelf === 'Currently Reading')) {       
      ...
      <div className="text-center mt-8 w-full ">
        {books.length === 0 ? (
          ...
          <div className="flex flex-col gap-2 mt-4">
            <div className={`grid ${selectedShelf === 'To Read' ? 'grid-cols-6' : 'grid-cols-7'} gap-4 px-16 py-4 font-bold bg-orange-300 text-brown-700`}>
              <div></div>
              <p>TITLE & AUTHOR</p>
            ...
            {books.map((book: any) => (
              <div key={book.id} className={`grid ${selectedShelf === 'To Read' ? 'grid-cols-6' : 'grid-cols-7'} gap-4 place-items-center px-16 py-8 bg-orange-600 text-brown-100`}>
                ...
                {selectedShelf != "To Read" && (
                  ...
                  {Array.from({ length: 5 }, (_, index) => (
                    <img
                      ...
                      onMouseEnter={() => handleMouseEnter(book.id, index + 1)}  
                      onMouseLeave={() => handleMouseLeave(book.id)}     
                      onClick={() => handleUpdateBook(book.id, 'rating', index + 1)} 
                    />
                  ))}
                </div>
                )}
